...
def process_raw(file, root, import_foldername, output):
    ...
    f = open(vgi,'r')
    line1 = f.readline(); line2 = f.readline(); XYZ = f.readline()
    f.close()
...
def export(pathOrig, volPath, type, x, y, z, format, fName, savePath):
    ...
    stringImport = """run("Raw...", "open=["""+doubleBackslash(volPath)+"""] image=["""+type+"""] width="""+x+""" height="""+y+""" offset=0 number="""+z+""" gap=0 little-endian use");"""
    print ("stringImport is " + stringImport)
    stringSaveStack = """run("Image Sequence... ", "format="""+format+""" name="""+fName+""" start=1 digits=4 save=["""+doubleBackslash(savePath)+"""]");"""
